...
import "./MACWallet.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "hardhat/console.sol";

/**
 * A sample factory contract for MACWallet
 * A UserOperations "initCode" holds the address of the factory, and a method call (to createAccount, in this sample factory).
 * The factory's createAccount returns the target account address even if it is already installed.
 * This way, the entryPoint.getSenderAddress() can be called either before or after the account is created.
 */
contract MACWalletFactory is Ownable {
    MACWallet public immutable accountImplementation;
    IEntryPoint entryPoint;
    address public aclModule;
    mapping(string => address) public usernameToAddress;

    event DeployedMACWallet(address account, string username, address owner);

    constructor(IEntryPoint _entryPoint, address _owner) {
        accountImplementation = new MACWallet(_entryPoint, address(this));
    ...
    function createAccount(
        ...
        uint256 salt,
        address ownerKey
    ) public returns (MACWallet ret) {
        address addr = getAddress(accountUsername, salt);
        uint codeSize = addr.code.length;
        if (codeSize > 0) {
            return MACWallet(payable(addr));
        }
        ...
        ret = MACWallet(
            payable(
                new ERC1967Proxy{salt: bytes32(salt)}(
                    ...
                    abi.encodeCall(MACWallet.initialize, (accountUsername))
                )
            )
        );

        emit DeployedMACWallet(address(ret), accountUsername, ownerKey);

        console.log("DeployedMACWallett %s %s", address(ret), accountUsername);
