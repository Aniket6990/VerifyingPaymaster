 ...
 */
contract MACWallet is BaseAccount, UUPSUpgradeable, Initializable {
    ...
    event MACWalletInitialized(
        IEntryPoint indexed entryPoint,
        string indexed username
    );
    event MACWalletOwnerChanged(
        IEntryPoint indexed entryPoint,
    ...
    constructor(IEntryPoint anEntryPoint, address _factory) {
        ...
        console.log("MACWallet account constructor", address(this));
    }
    ...
    function _onlyOwnerSignature(
        ...
        require(
            ...
            "account_acl: unauthorised request"
        );
    }
    ...
    function registerDeviceKey(
        ...
        require(
            ...
            "account_acl: device key already registered"
        );
        require(
            deviceKey != address(0) || deviceKey != owner,
            "account_acl: invalid device key"
        );

        deviceKeys[deviceKey] = true;
    }

    function removeDeviceKey(
        address deviceKey,
        bytes calldata signature
    ) external onlyOwner {
        _onlyOwnerSignature(signature, keccak256(abi.encode(deviceKey)));
        ...
        require(
            deviceKeys[deviceKey] == true,
            "account_acl: device key not registered"
        );
        require(
            deviceKey != address(0) || deviceKey != owner,
            "account_acl: invalid device key"
     ...
     * a new implementation of MACWallet must be deployed with the new EntryPoint address, then upgrading
    ...
    function _initialize(string calldata anUsername) internal virtual {
        ...
        emit MACWalletInitialized(_entryPoint, anUsername);
    }
